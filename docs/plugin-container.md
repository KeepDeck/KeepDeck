...
| `main.js`        | no       | The plugin's entry: a self-contained ESM module run in its logic realm (hooks, storage, events over RPC). Fixed name, no declaration — present means a logic realm boots, absent means pure UI. No externals — bundle everything, including `@keepdeck/plugin-guest`. |
...
- every declared dock tab has its `<tabId>.html`.
