...
/**
 * The Run tab: one command — one row. Each row fuses a saved command with its
 * live state for the CURRENT target: the state glyph on the left doubles as the
 * control — visible at rest while running (stop), on hover otherwise (run / run
 * again) — instances in other targets indent as child rows, and re-running
 * replaces a dead session instead of piling a new one. The command form
 * collapses behind the section header's "+" (the workspaces-rail idiom) and
 * opens above the list. Everything here drives the run manager; nothing touches
 * the agent grid. Presets live in the plugin's per-workspace storage slot
...
export function RunTab({ workspace, selectedPaneId }: DockTabProps) {
  ...
  /** Launch a preset — in the current target, or a specific one (a child row's
   * re-run). Always sends the preset's CURRENT command: the manager's
   * replace-on-relaunch picks it up, so an edited command applies on the next
   * run instead of resurrecting the old snapshot. */
  const launchPreset = (
   ...
   * except RUNNING, whose stop sits beside the dot at rest: a live process must
   * always show its off switch, and the dot stays a status light. `rerun`
   * overrides the dead-session action (preset rows relaunch with the preset's
   * current command); without it, the snapshot restarts (orphans — their old
   * command is all that's left of them). */
  const glyph = (
      ...
      case "running":
        return (
          <span className="run__g run__g--live">
            ...
            {act("Stop", () => manager.stopRun(s.id), <StopFillIcon />)}
          </span>
        );
      case "stopping":
        return (
      ...
      case "exited":
        return (
          <span className="run__g run__g--actable">
            ...
            {act(
              "Run again",
              rerun ?? (() => void manager.restartRun(s.id)),
              <PlayFillIcon />,
            )}
          </span>
        );
                    ...
                    {row.preset ? (
                      <>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            savePresets(removePreset(presets, row.preset!.id));
                            // Dead sessions of a deleted command are swept — a
                      ...
                      // An orphan: its preset is gone — only the session itself
                      // can be dismissed (killing it if alive).
                      <button
                        type="button"
                        className="run__act"
                        onClick={(e) => {
                          e.stopPropagation();
                          manager.removeRun(row.session!.id);
                        }}
          ...
          <div className="run__logcap" title={`${shown.command} · ${shown.worktree}`}>
            ...
            {shown.status.kind === "running" && (
              <button
                ...
                onClick={() => manager.stopRun(shown.id)}
