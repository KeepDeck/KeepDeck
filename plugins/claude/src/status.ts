 ...
 *   The host closes that window itself — the keystroke that answers goes
 *   through `writePane`, and `agentStatusChannel` folds it as `resumed`
 *   when it arrives (codex's normalizer carries the measured proof that
 *   no CLI reports an approval answer). Mid-turn repeats are absorbed by
 *   the reducer without an emit, so per-tool volume costs nothing
 *   downstream. Subagent tool calls arrive here too — see the case for
 *   why they are NOT filtered.
 *
