 ...
 * `--settings` outranks every file on disk — so this TAKES the statusLine
 * slot. The reporter gives it back by resolving the user's own statusLine at
 * runtime and delegating to it (kd-usage-statusline.sh); nothing on disk is
 * ever touched. SessionStart fires on startup/resume/clear/compact; the
 * statusLine command runs event-driven on every status update (rate_limits,
 * cost, context_window on stdin). */
