 ...
 * Dormant panes count: they revive right back into their directory. This is
 * what blocks the "+ Agent" dialog from attaching a second agent to a worktree
 * one pane already runs in (two agents in one dir stomp each other's files
 * and git state).
 */
export function paneOccupyingPath(
  ...
  for (const ws of workspaces) {
    for (const [index, pane] of ws.panes.entries()) {
      if (pane.cwd && normalizePath(pane.cwd) === wanted)
        return { ws, pane, index };
    }
  }
  return null;
}
