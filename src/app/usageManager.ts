...
/**
 * The owner of live usage state. Verified bridge reports funnel through
 * [`UsageManager.report`] (the channel hook authenticates tokens BEFORE
 * calling — this store never sees an unverified payload); `useUsage` reads
 ...
 * ([`UsageManager.retainPanes`]). Everything is runtime-only, never
 * persisted here (persistence subscribes from outside).
 *
 * A FACTORY, like the status tracker beside it: the app's one instance
 * lives in `createAppRuntime` and reaches consumers as a value; each test
 * builds a fresh instance instead of resetting a shared one.
 *
...
export interface UsageManager {
  ...
  registerNormalizer(agentId: string, normalizer: UsageNormalizer): () => void;
   ...
   * a replay of the EXISTING session file at arm time. Every tailed event
   * has an honest `sourceAt` plus `sourceMtimeMs` fallback; both replay and
   * live account claims use that provenance so delivery order cannot beat a
   ...
   * every account claim. */
  setAccount(provider: string, account: AccountUsage): void;
  ...
  retainPanes(liveIds: ReadonlySet<string>): void;
  /** Start a pane's telemetry lifetime over. Used when its session
   * generation changes or its process is deliberately retired. Account
   * state survives: it describes the provider account, not this pane. Live
   * provenance must reset too so a resumed session's catch-up can seed the
   * empty pane again. */
  clearPane(paneId: string): void;
  /** Accept a binding for a new session generation without erasing a report
   * from that same generation if filesystem delivery happened to overtake
   * the earlier `session.bound` envelope. */
  beginPaneSession(paneId: string, sessionId: string): void;
...
export function createUsageManager(): UsageManager {
   ...
   * live source already said. Gating on map membership instead of
   * provenance once dropped the second half of every replay (review
   * finding). */
  ...
  const manager: UsageManager = {
    ...
    report(paneId, payload, at = Date.now()) {
      ...
      if (result.pane) {
        ...
        // Merged, not replaced: codex splits model and numbers across
        // events; Claude splits live cost/limits from transcript token
        // cumulatives.
    ...
    beginPaneSession(paneId, sessionId) {
      if (panes.get(paneId)?.sessionId === sessionId) return;
      manager.clearPane(paneId);
    },
