 ...
 * no turn edge yet. A NARROW selector over the status tracker: the map
 * holds one stable object per pane between changes, so
...
export function usePaneActivity(paneId: string): PaneActivity | undefined {
  return useSyncExternalStore(agentStatusTracker.subscribe, () =>
    agentStatusTracker.getSnapshot().panes.get(paneId),
  );
}
