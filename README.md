...
This downloads the latest build for your Mac (Apple Silicon only) and
...
- **Usage without mixed meanings** — the top bar shows provider account-limit
  windows only. Per-session tokens and cost have their own top-bar
  **Usage statistics** screen with 24h/7d/30d/90d views, model/session
  breakdowns and 90-day local history. OpenCode contributes pane telemetry but
  no account-limit chip.
